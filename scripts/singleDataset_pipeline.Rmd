...
```{r plot-featureRNA, fig.align='center'}
...
ggplot(SO@meta.data, aes(nCount_RNA, nFeature_RNA, colour = percent.mt)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        text = element_text(size = 20)) +
  ylab('nFeature_RNA') +
  geom_hline(aes(yintercept = min.feat), color="#56B4E9", size=1) +
  geom_vline(aes(xintercept = max.counts), color="purple", size=1) +
  geom_vline(aes(xintercept = min.counts), color="green", size=1) +
  ggtitle(paste0("Number of read counts and features within each cells\n", SO@project.name)) +
  xlab('Number of read counts') +
  ylab('Number of features')
```
...
SO <- subset(SO,
             features=which(df$Nbr_of_cells > min.cells),
             subset = percent.mt < mito.high & percent.ribo > ribo.low & nCount_RNA > min.counts & nCount_RNA < max.counts & nFeature_RNA > min.feat)
```
...
### FeaturePlot of the markers with cluster labels  {.tabset .tabset-pills .tabset-fade}
...
topMarkers = by( markers.sign, markers.sign[["cluster"]], function(x)
{
  ...
  # Return top ones
  return( if(is.null(  top.markers )) x else head( x, n =  top.markers ));
});
