---
title: "01_Data preprocessing - Analysis datasets" %A% analysis datasets c'est pas très claire, mettre un titre plus explicite?
...
The goal is to analyze and compare the data obtained in our lab with the data retrieved from the Rossi et al. article. %A% mais notre goal c'est paas que de comparer ? On a l'impression que si vu la façon dont la phrase est formulée. Peut etre qu'il faut juste ajouter la description de notre dataset avant pour clarifier.
...
This directory is divided in 5 subdirectories: %A% "this" directory refers to 'inputDData' ?
...
-   1 directory contains the input tables provided by the reference Rossi et al. (`InputTables`). %A% c'est quoi ces tables ? C'est la liste données plus haut?
...
-   the replicate value, also present in the metadata and%A% c'est pas number plutot que value ici?
...
Now, all the sub-datasets are prepared for the analysis. %A% cette analyse concerne uniquement les lab data ou aussi les reference data? Si c'est les deux, comme c'est indiqué plus bas, l'indiquer ici.
First, I will control the quality of the cells, after what I will realize the standard preprocessing workflow of Seurat. %A% "after what I will realize" pas clair, réaliser est un faux-ami en anglais. Si tu veux dire : après cela j'applique le workflow de preprocessing standard de Seurat, c'est pas vraiment ce qui et écrit
Then, I will be able to remove the doublets using DoubletFinder library (PROVIDE SOURCE). %A% pourquoi "be able"? Tu pourrais pas le faire si tu avais pas appliqué les étapes précédentes?
...
Thus, they are stored in a list. %A% je comprends pas pourquoi "ainsi ils sont stockés dans une liste"?
...
Now, all sub-datasets Seurat objects are stored into a list. %A% c'est pas une répétition de ce qui est plus haut ?
...
It is possible that the data were already filtered. %A% tu peux pas le voir dans le code qu'ils donnent sur github?
...
Added to this one, I will also remove the cells with a number of read counts higher than 150.000 (purple line).%A% Je comprends pas la formulation "Added to this one, "
...
Furthermore, the higher threshold leads to filter out some outliers either in reference and laboratory sub-datasets. %A% il faudrait vérifier la consistance et nommer les données de Fabienne/Stéphane toujours de la même manière: il y a un mélange de MMG, MMG lab, lab, laboratory ... Je pense que MMG lab est le plus clair.
The 2 datasets of the laboratory fifth day do not reach such number read counts. %A% je pense que les datasets splittés par jour sont appelés sub-datasets ailleurs, garder toujours la meme terminologie pour éviter les confusions. Sinon ca veut dire que tu ne considères aucun des deux dataset jour 5 ?
...
Such features do not provide information on the cell heterogeneity. %A% ben si au contraire, du coup c'est pas très clair
...
In order to limit the number of dropouts, while still being able to identify small clusters, the features expressed in less than 8 cells will be removed. %A% mais le nombre de dropout ne se controle pas non ? Tu controles juste le filtre?
...
From the scaling step, a new slot named `scale.data` has been created. %A% c'est quoi "slot"?
...
All the downstream analysis was considering all the PCs. %A% all the 30 PCs? La formulation est un peu bizarre
...
For the sub-dataset `day_11` of the MMG laboratory, it appears that the best value for the resolution parameter should be at 1. %A% et pour les autres? Pourquoi préciser seulement pour celui-la?
I set the `Idents` of all sub-datasets to the clusters defined under the resolution level 1.%A% je comprends pas ce que c'est le idents ?
...
In fact, DoubletFinder is sensitive to the heterotypic doublets, but not to the homotypic ones (from [DoubletFinder github repository](https://github.com/chris-mcginnis-ucsf/DoubletFinder)). %A% ca veeut dire quoi "is sensitive"? Is able to detect ? Ca a l'air d'etre un defaut alors que je pense que c'est une qualité ?
...
In the Rossi et al. analysis, this rate was estimated at 7.6%. %A% which rate ? Pourqoui ici on donne la valeur a priori alors que pour les homotypic on fait un calcul sur les données?
...
Then, I cleaned the sub-datasets by studying the mitochondrial expression in the cells, and removing the outliers. %A% pas seulement, aussi filtre par feature count
